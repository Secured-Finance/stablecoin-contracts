...
contract CommunityIssuance is ICommunityIssuance, OwnableUpgradeable, CheckContract, BaseMath {
    ...
    // Issuance already accounted into totalProtocolTokenIssued, but not yet reflected
    // in StabilityPool's G reward sum.
    uint public pendingProtocolTokenIssuance;
    ...
    function startNewEmissionEpoch(uint256 additionalAmount) external onlyOwner {
        ...
        if (isFirstFunding) {
            ...
            uint settledIssuance = _issueProtocolToken();
            if (settledIssuance > 0) {
                pendingProtocolTokenIssuance = pendingProtocolTokenIssuance.add(settledIssuance);
                emit PendingProtocolTokenIssuanceUpdated(pendingProtocolTokenIssuance);
            }
    ...
    function issueProtocolToken() external override returns (uint) {
        ...
        uint newIssuance = _issueProtocolToken();
        uint pendingIssuance = pendingProtocolTokenIssuance;
        uint totalIssuance = pendingIssuance.add(newIssuance);

        if (pendingIssuance > 0) {
            pendingProtocolTokenIssuance = 0;
            emit PendingProtocolTokenIssuanceUpdated(0);
        }
