...
contract("CommunityIssuance - ProtocolTokenSupplyCap Update", async () => {
  ...
  it("startNewEmissionEpoch(): Correctly accumulates and resets pending issuance", async () => {
    const increaseAmount = initialProtocolTokenSupplyCap.div(4);
    ...
    const pendingInitial = await communityIssuanceTester.pendingProtocolTokenIssuance();
    assert.equal(pendingInitial.toString(), "0");

    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider);

    // Second epoch: Start new epoch WITHOUT withdrawal first
    // This should accumulate unissued tokens into pending
    await protocolToken.approve(communityIssuanceTester.address, increaseAmount);
    const tx1 = await communityIssuanceTester.connect(owner).startNewEmissionEpoch(increaseAmount);

    // Verify pending was accumulated (after 1 year, exactly 50% of initial cap should be unissued)
    const pendingAfterSecond = await communityIssuanceTester.pendingProtocolTokenIssuance();
    const expectedPendingAfterSecond = initialProtocolTokenSupplyCap.div(2);
    assert.isTrue(pendingAfterSecond.gt(0));
    assert.equal(expectedPendingAfterSecond.mul(10000).div(pendingAfterSecond).toString(), "10000");

    // Verify event was emitted
    const receipt1 = await tx1.wait();
    const pendingEvent1 = receipt1.events.find(
      (e) => e.event === "PendingProtocolTokenIssuanceUpdated",
    );
    assert.isDefined(pendingEvent1);
    assert.equal(
      pendingEvent1.args.pendingProtocolTokenIssuance.toString(),
      pendingAfterSecond.toString(),
    );

    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider);

    await protocolToken.approve(communityIssuanceTester.address, increaseAmount);
    const tx2 = await communityIssuanceTester.connect(owner).startNewEmissionEpoch(increaseAmount);

    // Verify pending accumulated further
    // After second epoch: new cap = remaining unissued (50% of initial) + increaseAmount
    // After 1 more year: exactly 50% of new cap remains unissued
    const pendingAfterThird = await communityIssuanceTester.pendingProtocolTokenIssuance();
    const supplyCapAfterSecond = initialProtocolTokenSupplyCap.div(2).add(increaseAmount);
    const newUnissued = supplyCapAfterSecond.div(2);
    const expectedPendingAfterThird = pendingAfterSecond.add(newUnissued);
    assert.isTrue(pendingAfterThird.gt(pendingAfterSecond));
    assert.equal(expectedPendingAfterThird.mul(10000).div(pendingAfterThird).toString(), "10000");

    // Verify event was emitted
    const receipt2 = await tx2.wait();
    const pendingEvent2 = receipt2.events.find(
      (e) => e.event === "PendingProtocolTokenIssuanceUpdated",
    );
    assert.isDefined(pendingEvent2);
    assert.equal(
      pendingEvent2.args.pendingProtocolTokenIssuance.toString(),
      pendingAfterThird.toString(),
    );

    // Withdraw to trigger pending reset
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider);
    await stabilityPool.connect(A).withdrawFromSP(0);

    // Verify pending was reset to 0
    const pendingAfterWithdraw = await communityIssuanceTester.pendingProtocolTokenIssuance();
    assert.equal(pendingAfterWithdraw.toString(), "0");
  });
});
